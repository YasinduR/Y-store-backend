...
export class ItemController {
  ...
  async createItem(req: Request, res: Response) {
    try {
      const {  name, description, stock,price,special_price } = req.body;
      const item = await itemService.createItem( name, description, stock,price,special_price);
  ...
  async updateItem(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const {name, description, stock,price,special_price } = req.body;
      const item = await itemService.updateItem(Number(id), name, description, stock,price,special_price);
      res.json(item);
    } catch (error) {
  ...
  async getAllItems(req: Request, res: Response) {
    try {
      const users = await itemService.getAllItems();
      res.json(users);
    } catch (error) {
...
// only Special_price will be updated acc to dicount provided in json body
  async applyDiscount(req: Request, res: Response) {
    try {
      ...
      const updatedItem = await itemService.updateItem(Number(id), item.name, item.description, item.stock, item.price, special_price);
